...
class PointHistoryQueryServiceTest {
    ...
    @Test
    void getPointHistories_ShouldReturnCorrectResponse() {
        // given
        ...
        when(assetHistoryMapper.findPointHistories(request)).thenReturn(mockList);
        when(assetHistoryMapper.countPointHistories(request)).thenReturn(totalCount);

        // when
        AssetHistoryListResponse response = pointHistoryQueryService.getPointHistories(request);
        ...
        verify(assetHistoryMapper, times(1)).findPointHistories(request);
        verify(assetHistoryMapper, times(1)).countPointHistories(request);
    }
}