...
const mentors = computed(() => {
  let filtered = originalMentors.value;

  if(searchParams.techStack !== null || searchParams.userName !== null){
    filtered = filtered.filter(
        (mentor) =>
            mentor.nickname.includes(searchParams.userName) ||
            mentor.techstackName.includes(searchParams.techStack)
    );
  }

  const sorted = [...filtered].sort((a, b) => {
    if (searchParams.coffeechatPrice !== null) {
      if (searchParams.coffeechatPrice === 'ASC') {
        return a.price - b.price
      }
      return b.price - a.price

    } else if (searchParams.temperature !== null) {
      if (searchParams.temperature === 'ASC') {
  ...
  pagination.totalPages = Math.ceil(sorted.length / ITEMS_PER_PAGE);
  return sorted;
})
...
const paginatedMentors = computed(() => {
  ...
  return mentors.value.slice(start, end);
});
// 프론트용 구현 끝
